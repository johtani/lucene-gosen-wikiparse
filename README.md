...
# 全オプションを指定
.\gradlew runParser --args="--old-jar lib/6.0.1 --new-jar lib/6.2.1 --input data/jawiki.xml --max-records 100 --format both --workers 4 --queue-size 2000"
...
- `--workers`, `-w`: 形態素解析のワーカースレッド数（デフォルト: `1`）
- `--queue-size`, `-q`: 並列処理の最大インフライト件数（デフォルト: `1000`）
- `--help`, `-h`: ヘルプメッセージを表示
- `--version`, `-V`: バージョン情報を表示

**Note**: ディレクトリを指定すると、そのディレクトリ内の全てのJARファイルが自動的に読み込まれます。
**Note**: レポート出力順は入力順を維持します。まず `--workers 1` で基準を取り、その後 `--workers 2` 以上で性能比較する運用を推奨します。
...
# 全オプションを指定
.\gradlew runWiki40bParser --args="--old-jar lib/6.0.1 --new-jar lib/6.2.1 --input ./data/wiki40b-ja/test.parquet --max-records 100 --format both --workers 4 --queue-size 2000"
